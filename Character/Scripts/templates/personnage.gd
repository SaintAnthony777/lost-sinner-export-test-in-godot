...
@onready var character: character_mesh =$Rogue_Hooded
...
func _physics_process(delta: float) -> void:
	...
	moves_logics(velocity,is_aiming)
	move_and_slide()
...
###### Fonctions ICI ####
func moves_logics(vel:Vector3,isaiming:bool):
	var ground_speed=vel.length()
	if is_on_floor():
		if ground_speed>=.2:
			if !isaiming:
				character.set_to_motion("Run")
			else: 
				strafing_direction()
				character.set_to_strafes(strafe_direction)
		else:
			character.set_to_motion("Idle")
	else :
		if vel.y>0:
			character.set_to_jump_falls_dash("jump_start")
		else : 
			character.set_to_jump_falls_dash("falls")
...
func character_moving(dir:Vector3):
	if is_aiming and Input.is_action_pressed("Bas"): SPEED = 3.0 
	else : SPEED = 7.0
	if dir:

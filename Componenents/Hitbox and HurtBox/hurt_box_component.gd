...
@onready var self_group:Array=self.owner.get_groups()
func _init() -> void:
	collision_layer=5
	collision_mask=4
...
func _on_area_entered(area: Area3D) -> void:
	if area is HitBoxComponent and self.owner!=area.owner:
		var hitter_group_list:Array=area.owner.get_groups()
		for group_name in self_group:
			if group_name and group_name in hitter_group_list:
				return
